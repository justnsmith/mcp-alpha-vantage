...
class TestCompareStocks:
    ...
    @patch("server.client")
    def test_compare_stocks_success(self, mock_client):
        """compare_stocks returns JSON with per-stock metrics and highlights."""
        mock_client.get_batch_quotes.return_value = _SAMPLE_QUOTES

        result = json.loads(compare_stocks("AAPL,MSFT,TSLA,GOOGL"))
    ...
    @patch("server.client")
    def test_compare_stocks_intraday_range(self, mock_client):
        ...
        result = json.loads(compare_stocks("AAPL"))
    ...
    @patch("server.client")
    def test_compare_stocks_most_volatile(self, mock_client):
        ...
        result = json.loads(compare_stocks("AAPL,MSFT"))

        assert result["most_volatile"]["symbol"] == "MSFT"

    def test_compare_stocks_no_symbols(self):
        """compare_stocks returns an error when given an empty string."""
        result = json.loads(compare_stocks(""))
        assert "error" in result

    def test_compare_stocks_too_many_symbols(self):
        """compare_stocks enforces the 50-symbol limit."""
        symbols = ",".join([f"SYM{i}" for i in range(51)])
        result = json.loads(compare_stocks(symbols))
        assert "error" in result

    @patch("server.client")
    def test_compare_stocks_rate_limit(self, mock_client):
        """compare_stocks propagates rate-limit errors gracefully."""
        from client import RateLimitError

        mock_client.get_batch_quotes.side_effect = RateLimitError("limit hit")
...
class TestScreenStocks:
    ...
    @patch("server.client")
    def test_screen_gainers_only(self, mock_client):
        """direction='gainers' excludes stocks that are flat or down."""
        mock_client.get_batch_quotes.return_value = _SAMPLE_QUOTES

        result = json.loads(screen_stocks("AAPL,MSFT,TSLA,GOOGL", direction="gainers"))
    ...
    @patch("server.client")
    def test_screen_losers_only(self, mock_client):
        """direction='losers' excludes stocks that are flat or up."""
        mock_client.get_batch_quotes.return_value = _SAMPLE_QUOTES

        result = json.loads(screen_stocks("AAPL,MSFT,TSLA,GOOGL", direction="losers"))
    ...
    @patch("server.client")
    def test_screen_min_change_percent(self, mock_client):
        """min_change_percent filters out stocks below the threshold."""
        mock_client.get_batch_quotes.return_value = _SAMPLE_QUOTES

        # TSLA ≈ +4.17 %, AAPL ≈ +1.30 %  → only TSLA should pass 2 %
        result = json.loads(screen_stocks("AAPL,MSFT,TSLA,GOOGL", min_change_percent=2.0))
    ...
    @patch("server.client")
    def test_screen_min_volume(self, mock_client):
        """min_volume filters out stocks with insufficient trading activity."""
        mock_client.get_batch_quotes.return_value = _SAMPLE_QUOTES

        # Only TSLA (80 M) and AAPL (52 M) exceed 25 M threshold
        result = json.loads(screen_stocks("AAPL,MSFT,TSLA,GOOGL", min_volume=25_000_000))
    ...
    @patch("server.client")
    def test_screen_combined_criteria(self, mock_client):
    ...
    @patch("server.client")
    def test_screen_no_matches(self, mock_client):
        """screen_stocks returns an empty list when nothing passes the filter."""
        mock_client.get_batch_quotes.return_value = _SAMPLE_QUOTES

        result = json.loads(screen_stocks("AAPL,MSFT,TSLA,GOOGL", min_change_percent=50.0))

        assert result["matched_count"] == 0
        assert result["matched"] == []

    @patch("server.client")
    def test_screen_criteria_applied_string(self, mock_client):
    ...
    def test_screen_invalid_direction(self):
        """screen_stocks returns an error for an invalid direction value."""
        result = json.loads(screen_stocks("AAPL,MSFT", direction="sideways"))
        assert "error" in result

    def test_screen_no_symbols(self):
        """screen_stocks returns an error when given an empty string."""
        result = json.loads(screen_stocks(""))
        assert "error" in result
...
class TestGetPortfolioSnapshot:
    ...
    def test_snapshot_success(self, mock_client):
        ...
        result = json.loads(get_portfolio_snapshot("AAPL:10,MSFT:5"))
    ...
    @patch("server.client")
    def test_snapshot_top_contributor_and_detractor(self, mock_client):
        ...
        result = json.loads(get_portfolio_snapshot("AAPL:10,MSFT:5,TSLA:20"))
